...
if __name__ == '__main__':
  ...
  color , dotime = ['r','b','k'] , 0
  for K in [20,30,40]: 
    
    data = np.load(f'DA_EnKF_xa_y0_alpha1normal_32_loc2_False-inflationAlpha0.1_K{K}_.npy')# (2K,t,x,y)
    ...
    plt.plot(data_RMSE_1000_list, n_index1000, f'{color[dotime]}d', label=f'K={K},Time: 1000')
    plt.plot(data_RMSE_4000_list, n_index4000, f'{color[dotime]}o', label=f'K={K},Time: 4000')
  ...
  for K in [20,30,40]: 
    ...
    plt.plot(timeline, error(data_fft_small , data_xa_determine ),color[dotime],label=f'K={K}') 
  ...
  plt.grid(alpha=0.5, linestyle='--', linewidth=0.7)
  plt.tight_layout()
  plt.savefig(f'plot/K-timeline_error_small.png')
