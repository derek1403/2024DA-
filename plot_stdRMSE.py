...
if __name__ == '__main__':
  ...
  color , dotime = ['r','b','g','k'] , 0
  for localization_distance in [2,5,10,15]: 
    
    data = np.load(f'DA_EnKF_xa_y0_alpha1normal_32_loc{localization_distance}_False-inflationAlpha0.1_K20_.npy')# (2K,t,x,y)
    ...
    plt.plot(data_RMSE_1000_list, n_index1000, f'{color[dotime]}d', label=f'L={localization_distance},Time: 1000')
    plt.plot(data_RMSE_4000_list, n_index4000, f'{color[dotime]}o', label=f'L={localization_distance},Time: 4000')
  ...
  # Show plot
  plt.tight_layout()
  plt.savefig(f'plot/localization_distance.png')
  plt.show()
  ...
  for localization_distance in [2,5,10,15]: 
    ...
    plt.plot(timeline, error(data_fft_small , data_xa_determine ),color[dotime],label=f'L={localization_distance}') 
  ...
  plt.title('localization distance') 
  plt.xlabel('timeline') 
  plt.ylabel('RMSE')
  
  plt.grid(alpha=0.5, linestyle='--', linewidth=0.7)
  plt.tight_layout()
  plt.savefig(f'plot/localization_distance-timeline_error_small.png')
