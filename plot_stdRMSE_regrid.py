...
if __name__ == '__main__':
  ...
  for regrid in [32,64]:
    ...
    data = np.load(f'DA_EnKF_xa_y0_alpha1normal_{regrid}_loc2_False-inflationAlpha0.1_K40_.npy')# (2K,t,x,y)
    ...
    plt.plot(data_RMSE_1000_list, n_index1000, f'{color[dotime]}d', label=f'regrid={regrid},Time: 1000')
    plt.plot(data_RMSE_4000_list, n_index4000, f'{color[dotime]}o', label=f'regrid={regrid},Time: 4000')
  ...
  for regrid in [32,64]: 
    ...
    plt.plot(timeline, error(data_fft_small , data_xa_determine ),color[dotime],label=f'regrid={regrid}') 
  ...
  plt.grid(alpha=0.5, linestyle='--', linewidth=0.7)
  plt.tight_layout()
  plt.savefig(f'plot/regrid-timeline_error_small.png')
